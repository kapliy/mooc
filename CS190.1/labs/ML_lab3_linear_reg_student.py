# Databricks notebook source exported at Thu, 9 Jul 2015 02:24:30 UTC
# MAGIC %md
...
averageTrainYear = (parsedTrainData
                    .map(lambda x: x.label).sum() * 1.0 / nTrain)
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return np.sqrt(labelsAndPreds.map(lambda x: squaredError(x[0], x[1])).sum() * 1.0 / labelsAndPreds.count())
...
def twoWayInteractions(lp):
    ...
    return LabeledPoint(lp.label, np.hstack((lp.features, 
                                             [v[0]*v[1] for v in itertools.product(lp.features, repeat=2)])))
