# Databricks notebook source exported at Thu, 16 Jul 2015 17:12:05 UTC
# MAGIC %md
...
def parsePoint(point):
    ...
    """
    return [ (i, v) for i, v in enumerate(point.split(',')[1:]) ]
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    Returns:
        SparseVector: A SparseVector of length numOHEFeats with indicies equal to the unique
            ...
            with values equal to 1.0.
    """
    keys = set(rawFeats).intersection(OHEDict.keys())
    return SparseVector(numOHEFeats, {OHEDict[key]: 1.0 for key in keys})
...
classOneFracTrain = OHETrainData.filter(lambda x: x.label == 1).count() * 1.0 / OHETrainData.count()
...
def parseHashPoint(point, numBuckets):
    ...
    Returns:
        LabeledPoint: A LabeledPoint with a label (0.0 or 1.0) and a SparseVector of hashed
            features.
    """
    raw_feats = parsePoint(point)
    hmap = hashFunction(numBuckets, raw_feats)
    return LabeledPoint(point.split(',')[0], SparseVector(numBuckets, hmap))
...
def computeSparsity(data, d, n):
    ...
    """
    return data.map(lambda x: len(x.features.indices) * 1.0 / len(x.features)).mean()
...
classOneFracTest = hashTestData.filter(lambda x: x.label == 1).count() * 1.0 / hashTestData.count()
logLossTestBaseline = hashTestData.map(lambda x: computeLogLoss(classOneFracTest, x.label)).sum() / hashTestData.count()
